...
static void UART2_PrintPCM8Debug(void);
...
/**
 * @brief 8サンプル平均済みPCMをmainコンテキストから出力する。
 *        割込み内でUART待ちを行わないため、16kHzのPWM更新を阻害しない。
 */
static void UART2_PrintPCM8Debug(void)
{
  int16_t left;
  ...
  if (AUDIO_DebugPop(&left, &right) != 0U)
  {
    int len = snprintf(line, sizeof(line), "PCM8 L=%d R=%d\\r\\n", (int)left, (int)right);
    if (len > 0)
    {
...
void SystemClock_Config(void)
{
  ...
  */
  ...
  /* USB FSには48MHzが必要。SYSCLK=72MHzのPLLを1.5分周して供給する。 */
