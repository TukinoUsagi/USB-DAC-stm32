...
/**
 * @brief TIM_Base MSP Initialization (TIM3/TIM4用: 出力ピンなし)
  * @retval None
  */
void HAL_TIM_Base_MspInit(TIM_HandleTypeDef *timHandle)
{
  ...
  {
    /*
     * TIM4 は USB Audio のPCMサンプルレート用タイマー。
     * HAL_TIM_Base_Init() の内部でこのMSP関数が呼ばれるため、
     * ここでクロックを有効化してから PSC/ARR を設定させる。
     */
